...
def run(args):
    # Setup the paths
    dataset = args.dataset  # 这里是dataset/room_raw125
    images = dataset / "images/"  # dataset/room_raw63/images/
    sift_sfm = dataset / "3D-models/room_raw125"  # dataset/aachen_v1.1/3D-models/room_raw125，官方提供的 COLMAP SfM 模型

    outputs = args.outputs  # the path where everything will be saved, 这里是outputs/room_raw63_render62
    ...
    results = (
        outputs / f"room_raw125_hloc_superpoint+superglue_netvlad{args.num_loc}.txt"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="datasets/room_raw125",
        help="Path to the dataset, default: %(default)s",
    )
    parser.add_argument(
        "--outputs",
        type=Path,
        default="outputs/room_raw125",
