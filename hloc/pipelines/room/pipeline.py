...
def run(args):
    ...
    images = dataset / "images_upright/"  # dataset/aachen_v1.1/images_upright/
    sift_sfm = dataset / "3D-models/aachen_v_1_1"  # dataset/aachen_v1.1/3D-models/aachen_v_1_1，官方提供的 COLMAP SfM 模型

    outputs = args.outputs  # the path where everything will be saved, 这里是outputs/aachen_v1.1
...
#     返回生成的.h5 文件的路径，存着几千张图的 Keypoints 和 Descriptors。
    ...
    features = Path("outputs\\aachen_v1.1\\feats-superpoint-n2048-rmax1024.h5")
...
#   生成定位用的“匹配对”: 对于每一张 Query 图，计算它和所有 Database 图的全局特征相似度，选出 Top-50（由 args.num_loc 控制）最像的图。
    ...
    # 对 Query 图和它找到的 Top-50 个“嫌疑人”进行精细匹配
